$(function () {
    ...
    $('#edit_sell').click(function () {
        $.ajax({
            url:'management/ajax_expense.php',
            type: "post",
            data: {
                'id':$('#edit_id').val().trim(),
                'reason_edit':$('#reasons_edit').val().trim(),
    ...
    $(document).on( "click", '.edit_button',function() {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $('#edit_id').val(info.data.Option);
                $('#reasons_edit').val(info.data.Reasons);
                $('#amount_edit').val(info.data.Amount);
                $('#modal-title').html('ID-'+info.data.Option+' '+info.data.Date+' Time-'+tConvert(info.data.Time));
            },
    ...
    var expenseTable = $('#expense').DataTable({
        ...
        columns:
            [
                ...
                { "data": "Option",
                    render: function ( data, type, row ) {
                        return '<button type="button" data-id="'+data+'" class="btn btn-rounded btn-info edit_button" data-toggle="modal" data-target="#modal-center-edit">\n' +
                            'Edit\n' +
    ...
    $('#submit_reason').click(function (e) {
        e.preventDefault();
        if($('#date').val() && $('#time').val() && $('#reasons').val() && $('#amount').val()){
            $.ajax({
                url:"management/ajax_expense.php",
                type:'post',
                data:{
