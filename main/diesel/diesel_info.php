...
if(logged_in()){
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        $sql= "SELECT id,date, time, m.manager_name,add_diesel.shift ,prev_stock, new_stock, total_stock, sell_quantity, buy_rate, invest, sell_rate, earn,profit from add_diesel inner join managerinfo m on add_diesel.manager = m.manager_id ORDER BY date desc limit 100";
        $result= query($sql);
        confirm($result);
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        if(isset($_POST['date']) && isset($_POST['time']) && isset($_POST['shift']) && isset($_POST['prevStock']) && isset($_POST['newStock']) && isset($_POST['totalStock']) && isset($_POST['sellQuantity']) && isset($_POST['buyRate']) && isset($_POST['investment']) && isset($_POST['sellRate']) && isset($_POST['profit']) && isset($_POST['earn']) && isset($_POST['investment'])){
            $sql = "INSERT INTO add_diesel (date, time,shift, prev_stock, new_stock, total_stock, sell_quantity, buy_rate, invest, sell_rate, profit, earn, manager)
VALUES ('" . clean($_POST["date"]) . "','" . clean($_POST["time"]) . "','" . clean($_POST["shift"]) . "','" . clean($_POST["prevStock"]) . "','" . clean($_POST["newStock"]) . "','" . clean($_POST["totalStock"]) . "','" . clean($_POST["sellQuantity"]) . "','" . clean($_POST["buyRate"]) . "','" . clean(roundUp($_POST["investment"])) . "','" . clean($_POST["sellRate"]) . "','" . clean(roundUp($_POST["profit"])) . "','" . clean(roundUp($_POST["earn"])) . "','" . clean($_POST["manager"]) . "')";
        ...
        elseif (isset($_POST['id']) && isset($_POST['managerEdit']) && isset($_POST['shiftEdit']) && isset($_POST['prevStockEdit']) && isset($_POST['newStockEdit']) && isset($_POST['totalStockEdit']) && isset($_POST['sellQuantityEdit']) && isset($_POST['buyRateEdit']) && isset($_POST['investmentEdit']) && isset($_POST['sellRateEdit']) && isset($_POST['earnEdit']) && isset($_POST['profitEdit'])){
            $sql = "UPDATE add_diesel SET manager = '" . clean($_POST["managerEdit"]) . "',shift = '" . clean($_POST["shiftEdit"]) . "',prev_stock = '" . clean($_POST["prevStockEdit"]) . "',new_stock='" . clean($_POST["newStockEdit"]) . "',total_stock='" . clean($_POST["totalStockEdit"]) . "',sell_quantity='" . clean($_POST["sellQuantityEdit"]) . "',buy_rate='" . clean($_POST["buyRateEdit"]) . "',invest='" . clean($_POST["investmentEdit"]) . "', sell_rate='" . clean($_POST["sellRateEdit"]) . "',earn='" . roundUp($_POST["earnEdit"]) . "',profit='" . roundUp($_POST["profitEdit"]) . "' where id='" . clean($_POST["id"]) . "'";
            $result = query($sql);
            confirm($result);
        }
        elseif (isset($_POST['id']) && isset($_POST['edit'])){
            $sql = "SELECT * from add_diesel where id='" . clean($_POST['id']) . "'";
            $result = query($sql);
            confirm($result);
        ...
        elseif(isset($_POST['id']) && isset($_POST['delete'])){
            $sql="DELETE FROM add_diesel where id='".clean($_POST["id"])."'";
            $result= query($sql);
            confirm($result);
        }
